...
protocol ResultadosBusquedaViewDelegate: AnyObject {
...
/// Muestra los resultados de la Busqueda UI (tabla o colección como sea necesario)
final class ResultadosBusquedaView: UIView {

    weak var delegate: ResultadosBusquedaViewDelegate?

    private var viewModel: ResultadosBusquedaViewModel? {
        didSet {
    ...
    private var locationCellViewModels: [LocacionTableViewCellViewModel] = []
    ...
    private func setUpTableView(viewModels: [LocacionTableViewCellViewModel]) {
    ...
    public func configure(with viewModel: ResultadoBusquedaViewModel) {
...
extension ResultadosBusquedaView: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return locationCellViewModels.count
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: LocacionTableViewCell.cellIdentifier,
                                                       for: indexPath) as? LocacionTableViewCell else {
            fatalError("Failed to dequeue LocacionTableViewCell")
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        delegate?.rmResultadosBusquedaView(self, didTapLocationAt: indexPath.row)
    }
}

// MARK: - CollectionView

extension ResultadosBusquedaView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let characterVM = currentViewModel as? PersonajeCollectionViewCellViewModel {
            // Celda Personaje
        ...
        // Episodio
        guard let cell = collectionView.dequeueReusableCell(
            withReuseIdentifier: PersonajeEpisodioCollectionViewCell.cellIdentifer,
            for: indexPath
        ) as? PersonajeEpisodioCollectionViewCell else {
            fatalError()
        }
        if let episodeVM = currentViewModel as? PersonajeEpisodioCollectionViewCellViewModel {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        switch viewModel.results {
        case .characters:
            delegate?.rmResultadosBusquedaView(self, didTapCharacterAt: indexPath.row)
        case .episodes:
            delegate?.rmResultadosBusquedaView(self, didTapEpisodeAt: indexPath.row)
        case .locations:
            break
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if currentViewModel is PersonajeCollectionViewCellViewModel {
            // Tamanio Personaje
...
extension ResultadosBusquedaView: UIScrollViewDelegate {
